...
export function initManuscript(targetId: string) {
  ...
  function step() {
    timer = null;
    if (paused || opIdx >= activeOps.length) return;
  ...
  function clearAll(done: () => void) {
    if (content.length === 0) { done(); return; }
  ...
  return {
    ...
    resume() {
      ...
      paused = false;
      if (timer === null && opIdx < activeOps.length) scheduleNext(1000);
    },
    switchTo(lang: 'es' | 'en') {
      if (switching) return;
      switching = true;
